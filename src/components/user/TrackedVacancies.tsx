...
const TrackedVacancies: React.FC = () => {
  const [trackedVacancies, setTrackedVacancies] = useState<TrackedVacancy[]>([]);
  ...
  const handleUntrack = async (vacancyId: string) => {
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {trackedVacancies.map((tracked) => (
          <div key={tracked.vacancyId} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow p-6 border border-gray-100">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex items-center text-gray-600">
                ...
                <span className="text-sm">Posted: {new Date(tracked.vacancyDetails.postedDate).toLocaleDateString()}</span>
              </div>
              
              {tracked.vacancyDetails.examDate && (
                <div className="flex items-center text-gray-600">
                  <Calendar className="h-4 w-4 mr-2" />
                  <span className="text-sm">Exam Date: {new Date(tracked.vacancyDetails.examDate).toLocaleDateString()}</span>
                </div>
              )}

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600">
                <div>
                  <span className="font-medium">Qualifications:</span> {tracked.vacancyDetails.qualifications}
                </div>
                <div>
                  <span className="font-medium">Age Range:</span> {tracked.vacancyDetails.ageRange}
                </div>
                <div className="sm:col-span-2">
                  <span className="font-medium">Required Stream:</span> {tracked.vacancyDetails.requiredQualificationStream}
                </div>
              </div>
            </div>
