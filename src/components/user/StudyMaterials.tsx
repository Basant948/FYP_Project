...
const StudyMaterials: React.FC = () => {
  ...
  const handleGenerateTest = async (id: string) => {
    try {
      const { data } = await studyService.generateTest(id);
      // For now, just show an alert with the test data
      // In a real app, you'd navigate to a test component
      alert(`Test generated with ${data.length} questions. This would open a test interface.`);
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6">
        {materials.map((material) => (
          <div key={material.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow p-6 border border-gray-100">
