...
interface VacancyCardProps {
  vacancy: Vacancy;
  onTrack: (vacancyId: string) => void;
  onRate: (vacancyId: string, rating: number) => void;
}

const VacancyCard: React.FC<VacancyCardProps> = ({ vacancy, onTrack, onRate }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 border border-gray-100">
      <div className="flex justify-between items-start mb-4">
        ...
        >
          {vacancy.tracked ? <BookmarkCheck className="h-5 w-5" /> : <Bookmark className="h-5 w-5" />}
        </button>
      </div>
